...
//	IRingSet rs;
	...
	String[] AssignedFragment;

	Hashtable<String, Double> htFragVal;
...
//			
	...
	/**
	 * adds hydrogens to m to create m2 This method assumes that the hydrogen counts
	 * have already been set by CalcFragmentEStates method
	 * 
	 * @param m
	 * @return
	 */
...
//		added by TMM-11/14/07:
		...
		this.getCount("CH2=C(CH3)C(=O)O-", "C([H])([H])=C(C([H])([H])([H]))C(=O)O", "Any", "Any", "O", "SssO", "",
				"Any");
		...
		// Doug TODO move before "Si [aromatic attach]":
		...
		this.getCount("-S(=O)(=O)- [nitrogen, aromatic attach]", "S(=O)(=O)", "Aromatic", "Any", "S", "SddssS", "",
				"N");
...
//		need for IGC50 chemicals like 137-40-6:
		...
		this.getCount("AN+", "N", "Aromatic", "Any", "N", "SaaaNp", "", "Any");// TODO: wont work unless SaaaNp Estate
																				// frag is added!
		...
		this.getCount("As", "[As]", "Any", "Any", "As", "Any", "", "Any");// TODO change name of frag to "[As]" to avoid
																			// name conflict with WHIM "As" descriptor
...
//		this.GetDiesterxCH2InteractionCount();
	...
	private void GetCOOHxCH2InteractionCount() {
		...
		this.htFragVal.put(fragdesc, val);
	...
	private void FindACAC(IAtomContainer m, IRingSet rs) {
		...
		for (int i = 0; i <= m.getAtomCount() - 1; i++) {
			...
			for (int j = 0; j < ca.size(); j++) {
				...
				if (Assigned[m.indexOf(caj)]) {
					continue;
				}

				if (!Fragment[m.indexOf(caj)].equals("SsaaC"))
					continue;
				...
				if (!EStateFragmentDescriptor.InSameAromaticRing(m, m.getAtom(i), caj, rs)) {
					...
					Assigned[m.indexOf(caj)] = true;
					AssignedFragment[m.indexOf(caj)] = "ACAC";
		...
		this.htFragVal.put(fragdesc, count);
	...
	private void FindANANSameRing(IAtomContainer m, IAtomContainer m2, IRingSet rs) {
		...
		iloop: for (int i = 0; i <= m.getAtomCount() - 1; i++) {
			if (Assigned[i])
				continue;
			if (!Fragment[i].equals("SsaaN") && !Fragment[i].equals("SaaN") && !Fragment[i].equals("SaaNH"))
				continue;

			List<IAtom> ca = m.getConnectedAtomsList(m.getAtom(i));

			jloop: for (int j = 0; j < ca.size(); j++) {
				IAtom caj = (IAtom) ca.get(j);
				if (!Fragment[m.indexOf(caj)].equals("SsaaN") && !Fragment[m.indexOf(caj)].equals("SaaN")
						&& !Fragment[m.indexOf(caj)].equals("SaaNH"))
					continue;
			...
			if (Fragment[i].equals("SaaNH") && Assigned[i]) {
				// assign hydrogen:
				List<IAtom> ca2 = m2.getConnectedAtomsList(m2.getAtom(i));

				for (int j = 0; j < ca2.size(); j++) {
					IAtom ca2j = (IAtom) ca2.get(j);
					if (ca2j.getSymbol().equals("H")) {
						Assigned[m2.indexOf(ca2j)] = true;
						break;
					}
				}
			}
		...
		this.htFragVal.put(fragdesc, countAN);
		dd.MW_Frag += 14.00674 * countAN;

		fragdesc = "ANH [attached to AN in same ring]";
		this.htFragVal.put(fragdesc, countANH);
		dd.MW_Frag += (14.00674 + 1.00797) * countANH;

	}

	private void getCount(String fragdesc, String smiles, String AttachType, String RingType, String AttachAtomSymbol,
			String EStateAttachAtom, String DontAssignSymbol, String AttachedToSymbol) {

		String checkdesc = "-S(=O)(=O)- [nitrogen, aromatic attach]";
		...
		if (!(htFragVal.get(fragdesc) instanceof Double)) {
			...
			this.htFragVal.put(fragdesc, 0.0);
		}
...
//			}
			...
			// System.out.println("");
			List<List<Integer>> u = null;

			try {
				if (fragment.getAtomCount() > 2) {
					List<List<RMap>> ll = uit.getSubgraphAtomsMaps(molecule, fragment);
			...
			if (fragdesc.equals(checkdesc) && Debug)
...
//					System.out.println(fragdesc+"\t"+MWfrag);	
				...
				for (int j = 0; j < mlist.size(); j++) {
					int atomnumber = mlist.get(j);

					if (fragdesc.equals(checkdesc) && Debug) {
						System.out.println((atomnumber + 1) + "\t" + molecule.getAtom(atomnumber).getSymbol());
					}
				}
...
//						continue;
					...
					if ((Assigned[atomnumber] == true)) {
						...
						if (fragdesc.equals(checkdesc) && Debug)
							System.out.println("Has assigned atom:" + atomnumber);
				...
				int AttachmentAtomNumber = this.GetAttachmentAtomNumber(AttachAtomSymbol, EStateAttachAtom, molecule,
						mlist);

				if (AttachmentAtomNumber == -1 && !EStateAttachAtom.equals("Any")) { // added
																						// !EStateAttachAtom.equals("Any")
																						// on 3/18/10
					if (fragdesc.equals(checkdesc) && Debug)
...
//				}
				...
				String InARing = this.InARing(AttachmentAtomNumber, molecule, ringset);

				boolean HaveAttached = false;

				if (!AttachedToSymbol.equals("Any")) {
					HaveAttached = this.IsAttachedToElement(AttachmentAtomNumber, molecule, mlist, AttachedToSymbol);

					if (fragdesc.equals(checkdesc) && Debug) {
						System.out.println("AttachmentAtomNumber=" + AttachmentAtomNumber + "\tsymbol="
								+ molecule.getAtom(AttachmentAtomNumber).getSymbol());
						System.out.println("AttachedToSymbol=" + AttachedToSymbol);
					}
				}
				...
				if (!AttachedToSymbol.equals("Any") && !HaveAttached) {
					if (fragdesc.equals(checkdesc))
						if (Debug)
							System.out.println("failed attached to symbol");
					continue;
				}
				...
				if (!RingType.equals("Any") && !RingType.equals(InARing)) {
					if (fragdesc.equals(checkdesc))
						if (Debug)
							System.out.println("failed ring type");
					continue;
				}

				// note Aromatic can be Aromatic,TwoAromatic, ThreeAromatic, AromaticOlefinic,
				// etc.
				...
				if (!AttachType.equals("Any") && !at.equals(AttachType)) {
					...
					if (AttachType.equals("Aromatic") && at.indexOf("Aromatic") > -1) {
						...
						if (fragdesc.equals(checkdesc)) {
							if (Debug)
								System.out.println("failed attach type2");
							if (Debug)
								System.out.println(at + "\t" + AttachType);
						}
...
//						continue iloop;
//					}
					if (molecule.getAtom(atomnumber).getSymbol().equals(DontAssignSymbol))
						continue;
				} // for j
...
//			System.out.println("Count="+count);
			...
			if (htFragVal.get(fragdesc) instanceof Double) {
				...
				double OldCount = htFragVal.get(fragdesc);
				count += OldCount;
				this.htFragVal.put(fragdesc, Double.valueOf(count));
			} else {
				dd.MW_Frag += count * MWfrag;
				this.htFragVal.put(fragdesc, Double.valueOf(count));
			}
...
//		IAtomContainer molecule=m2;
//		IRingSet ringset=rs2;
		...
		try {
			...
			for (int i = 0; i < rs2.getAtomContainerCount(); i++) {
				...
				List<Integer> AtomsWithNitro = this.GetRingAtomsWithNitroGroups(ring, m2);

				Collections.sort(AtomsWithNitro);

				int count = AtomsWithNitro.size();

				if (count != NumNitroGroups)
					continue;
				...
				if (NumNitroGroups == 2) {
					...
					if (val1 == pos1 && val2 == pos2) {
						this.htFragVal.put(fragdesc, 1.0);
						System.out.println(dd.ID + "\t" + fragdesc);
					}
				}
...
//				System.out.println(count);
	...
	private List<Integer> GetRingAtomsWithNitroGroups(IAtomContainer ring, IAtomContainer molecule) {

		List<Integer> list = new LinkedList<>();

		iloop: for (int i = 0; i < ring.getAtomCount(); i++) {
	...
	// used for finding things like propargylic alcohols (flag variables not
	// fragments)

	private void getCount2(String fragdesc, String smiles, String AttachType, String RingType, String AttachAtomSymbol,
			String EStateAttachAtom, String DontAssignSymbol, String AttachedToSymbol) {
...
//			if (Debug) System.out.println(fragdesc+"\t"+1+"\t"+MWfrag);
	...
	int GetAttachmentAtomNumber(String symbol, String EState, IAtomContainer molecule, List<Integer> mlist) {

		for (int tempatomnumber:mlist) {
	...
	String GetAttachmentAtomEStateFragment(String symbol, IAtomContainer molecule, List<Integer> mlist) {
		
		// Remove graphs that contain used atoms
		for (int tempatomnumber:mlist) {
			if (molecule.getAtom(tempatomnumber).getSymbol().equals(symbol)) {
	...
	boolean IsAttachedToElement(int AttachmentAtomNumber, IAtomContainer molecule, List<Integer> mlist, String attachsymbol) {
		...
		for (int j = 0; j < ca.size(); j++) {
			...
			if (!InFragment(mlist, molecule.indexOf(caj))) {
...
//		if (attachsymbol.equals("Cl")) System.out.println(count+"\t"+attachcount);	
		...
		if (count > 0 && attachcount == -1)
			return true;

		if (count == attachcount)
			return true;
		else
			return false;
	}

	boolean InFragment(List <Integer>atomlist, int AtomNumber) {		
		for (int num:atomlist) 
			if (num == AtomNumber) return true;
		return false;
	}

	private String InARing(int AttachmentAtomNumber, IAtomContainer molecule, IRingSet ringset) {
...
//		System.out.println("enter in a ring");
	...
	private List<List<Integer>> GetTwoAtomFragmentLists(IAtomContainer molecule, IAtomContainer fragment) {
		...
		iloop: for (int i = 0; i < molecule.getAtomCount(); i++) {
			...
			int AtomNum1 = molecule.indexOf(ai);
			...
			if (ai.getSymbol().equals(a0.getSymbol())) {
				...
				for (int j = 0; j < ca.size(); j++) {
					...
					int AtomNum2 = molecule.indexOf(caj);
	...
	private List<List<Integer>> GetSingleAtomFragmentLists(IAtomContainer molecule, IAtomContainer fragment) {
...
//			for (int i = 0; i < molecule.getAtomCount(); i++) {
	...
	public static List<List<Integer>> getUniqueSubgraphs(List<List<RMap>> subgraphs) {
		List<List<Integer>> unique = new ArrayList<>();

//        System.out.println("getting unique subgraphs");

		for (List<RMap> current : subgraphs) {
			List<Integer> ids = new ArrayList<>();

//            System.out.print(i+": ");
			for (RMap rmap : current) {
				ids.add(rmap.getId1());
...
//            System.out.println();
		...
		for (List<Integer> ids : hs)
			unique.add(ids);
		return unique;

	}

	public Hashtable<String, Double> Calculate(IAtomContainer m, IAtomContainer m2, IRingSet rs, IRingSet rs2, DescriptorData dd,
			String[] Fragment) {
...
//		System.out.println(Fragment.length);
		...
		for (int i = 0; i <= m.getAtomCount() - 1; i++) {
			if (Debug) {
				System.out.println((i + 1) + "\t" + Fragment[i] + "\t" + Assigned[i] + "\t" + AssignedFragment[i]);
			}
		}

		if (Debug)
			System.out.println("MW calc\t" + dd.MW_Frag);

		if (Debug) {
			for (String frag:htFragVal.keySet()) {
				double val = (Double) htFragVal.get(frag);
				if (val > 0)System.out.println(frag + "\t" + val);
			}
		}
	...
	private String GetAttachmentType(int AttachmentAtomNumber, IAtomContainer molecule, List<Integer> mlist) {
...
//		String at = "";
		...
		for (IAtom caj:ca) {
			...
			// int cajatomnumber = molecule.indexOf(ca[j]);
			...
			if (!InFragment(mlist, molecule.indexOf(caj))) {
				if (caj.getFlag(CDKConstants.ISAROMATIC)) {
					...
					if (caj.getSymbol().equals("C")) {
						List<IAtom> ca2 = molecule.getConnectedAtomsList(caj);

						for (IAtom ca2k:ca2) {
							
							if (!ca2k.equals(ai)) {
								if (ca2k.getSymbol().equals("C")
										&& molecule.getBond(caj, ca2k).getOrder().equals(IBond.Order.DOUBLE)) {
									OlefinicCount++;
								}
							}
						}
