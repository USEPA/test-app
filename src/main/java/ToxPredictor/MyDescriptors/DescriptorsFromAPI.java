...
public class DescriptorsFromAPI {
	...
	public static TreeMap<String,Double> calculateMordredDescriptors(String smiles) {
		...
		JsonArray jaDescriptors=jo.get("chemicals").getAsJsonArray().get(0).getAsJsonObject().get("descriptors").getAsJsonArray();

		
//		Hashtable<String,Double>htDescriptors=new Hashtable<>();
		TreeMap<String,Double>htDescriptors=new TreeMap<>();//autosorts by key
		
		int index=0;
		
		for(JsonElement je:jaDescriptors) {
			
			Double value=null;
			
			if(!je.isJsonNull()) {
				value=je.getAsDouble();
				htDescriptors.put(headers[index++], value);
			} 
//			System.out.println(index+"\t"+headers[index]+"\t"+value);
	...
	public static void main(String[] args) {
		...
		String smiles="[2H][13C](=O)C1=CC=CC=C1";
		TreeMap<String,Double>htDesc=DescriptorsFromAPI.calculateMordredDescriptors(smiles);
		System.out.println(gson.toJson(htDesc));
