...
public class XLogPDescriptor extends AbstractMolecularDescriptor implements IMolecularDescriptor {
    ...
    /**
     * @param checkAromaticity the checkAromaticity to set
     */
    ...
    /**
     * @param salicylFlag the salicylFlag to set
     */
    public void setSalicylFlag(boolean salicylFlag) {
        this.salicylFlag = salicylFlag;
    }
...
//                bondAtom1 = bond.getEnd();
                ...
                bondAtom0 = bond.getAtom(0);//TMM 5/9/17
    ...
    //5/9/17 TMM added this since this method was missing in IBond class for the cdk1.5 jar (need to create new jar file from git source files)
    private IAtom getOther(IBond bond,IAtom atom) {
        if (bond.getAtom(0) == atom)
            return bond.getAtom(1);
        else if (bond.getAtom(1) == atom)
    ...
    private int getPiSystemsCount(IAtomContainer ac, IAtom atom) {
        ...
        for (IAtom neighbour:neighbours) {
            ...
            for (int j = 0; j < bonds.size(); j++) {
                ...
                if (bond.getOrder() != IBond.Order.SINGLE && this.getOther(bond,neighbour) != atom
	 ...
	 * addExplicitHydrogensToSatisfyValency method (HydrogenAdder classe).
	 *
	 * Added by TMM 5/9/17 so that can use the m2 and rs2 objects previously
	 * generated in descriptor factory
	 *
	 *
	 * @param atomContainer
	 *            AtomContainer with hydrogens attached
	 * @param rs
	 *            RingSet corresponding to atomContainer
	 * @return XLogP is a double
	 */
	
	    public DescriptorValue calculate(IAtomContainer ac,IRingSet rs) {
