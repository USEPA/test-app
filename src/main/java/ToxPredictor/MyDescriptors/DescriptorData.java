...
//import ToxPredictor.misc.sarray;
//import ToxPredictor.misc.sortable;
...
//				}
			...
			fw.write("<br><a href=\"2d_descriptors.txt\">Descriptor text file</a> (\"|\" delimited, data listed in rows)\n");
...
//			System.out.println(test.ALOGP);
	...
	public Vector<String>getDescriptorValues() {
		...
		try {
			...
			if (!Error.contentEquals("OK")) return values;
...
// System.out.println(myFile.exists());
			...
			while (true) {
				...
				LinkedList ll=Utilities.Parse(Line,"\t");
...
//				 write autocorrelation of lag i / weighted by electronegativity
				...
				// write leverage-weighted autocorrelation of lag i / weighted by electronegativity
	...
	private void WriteFragments(FileWriter fw) {
		...
		if (!(FragmentList instanceof Hashtable)) {
		 ...
		 //   String fragnames=(String)this.FragmentList.get(0);
		    ...
		    Vector v=new Vector(FragmentList.keySet());
		    Collections.sort(v);
...
//
	...
	public String [] CreateVarList(boolean include2D,boolean includeFrag,boolean include3D,boolean includeRaghu3D, boolean includeKOWWIN) {
		ArrayList varList=new ArrayList();
		varList.add("CAS");
		varList.add("Tox");

		if (include2D) this.PutVarNamesInArray(varList,this.varlist2d);

		if (includeFrag){
			for (java.util.Enumeration e = FragmentList.keys(); e.hasMoreElements();) {
				varList.add((String) e.nextElement());
			}
		}

		if (include3D) this.PutVarNamesInArray(varList,this.varlist3d);

		if (includeRaghu3D) this.PutVarNamesInArray2(varList,this.strQM);

		if (includeKOWWIN) varList.add("KLOGP");

		//Convert ArrayList to String array:
		String [] varArray=new String [varList.size()];
		for (int i=0;i<varList.size();i++) varArray[i]=(String)varList.get(i);
		return varArray;
	...
	public Hashtable CreateDataHashtable(boolean include2D,boolean includeFrag,boolean include3D,boolean includeRaghu3D, boolean includeKOWWIN) {

		Hashtable ht=new Hashtable();

		ht.put("CAS",ID);
		ht.put("Tox",new Double(-9999));

		if (include2D) PutValuesInHashTable(ht,this.varlist2d);

		if (includeFrag) PutFragmentValuesInHashTable(ht);

		if (include3D) PutValuesInHashTable(ht,this.varlist3d);

		if (includeRaghu3D) PutValuesInHashTable2(ht,this.strQM);

		if (includeKOWWIN) ht.put("KLOGP",new Double(this.KLOGP));

		return ht;

	}

	public Hashtable<String,Object> CreateDataHashtable(String ToxFieldName,boolean include2D,boolean includeFrag,boolean include3D,boolean includeRaghu3D, boolean includeKOWWIN) {

		Hashtable<String,Object> ht=new Hashtable();

		ht.put("CAS",ID);
		ht.put(ToxFieldName,new Double(-9999));
	...
	public Hashtable CreateDataHashtableFDASubSet() {
	...
	public void DisplayDataHashtableValues(Hashtable ht,String [] varNames) {
	...
	private void PutFragmentValuesInHashTable(Hashtable ht) {
		Vector v=new Vector(FragmentList.keySet());
...
//		Collections.sort(v); //quick way to sort by frag names
	    ...
	    for ( Enumeration e = v.elements(); e.hasMoreElements();) {
			...
			ht.put(object_key,new Double(object_val));
	    }
	}
	 ...
	 */
	private void PutValuesInHashTable(Hashtable ht, String[] ArrayOfArrayNames) {
		try {
	 ...
	 * @param varList = array of array names
	 */
	private void PutValuesInHashTable2(Hashtable ht, String[] varList) {
		try {
...
//				}
	...
	private void PutVarNamesInArray(ArrayList varList, String[] ArrayOfArrayNames) {
		try {
			for (int i = 0; i <= ArrayOfArrayNames.length - 1; i++) {
				Field myField = this.getClass().getField(ArrayOfArrayNames[i]);
	...
	private void PutVarNamesInArray2(ArrayList varList, String[] varArray) {
	...
	public JsonObject toJSON() {
		...
		try {
			...
			JsonObject nonFragmentDescriptors=new JsonObject();//store as separate object to make converting back to DescriptorData
			...
			JsonObject fragmentDescriptors=new JsonObject();//store as separate object to make converting back to DescriptorData
...
//				System.out.println(dd.MW+"\t"+dd.MW_Frag);
			...
			//TODO- check desc values somehow?
			return dd;
	...
	public static DescriptorData loadFromJSON(String strJSON) {
		...
		try {
			...
			if (!dd.Error.equals("OK")) return dd;
...
//				System.out.println(dd.MW+"\t"+dd.MW_Frag);
	...
	public static DescriptorData toDescriptorData(JsonObject jo) {
		...
		try {
			...
			if (metadata.get("SMILES")!=null) dd.SmilesRan=metadata.get("SMILES").getAsString();
...
//						System.out.println(names[j]+"\t"+je.getAsInt());
			...
			for (int i = 0; i < dd.strFragments.length; i++) {//TODO probably should iterate through fragmentDescriptors Object
				JsonElement je=fragmentDescriptors.get(dd.strFragments[i]);
				double Val = je.getAsDouble();
				dd.FragmentList.put(dd.strFragments[i], Val);
	...
	public static Vector<String> getDescriptorNames() {
		...
		try {
			...
			for (int i = 0; i < dd.strFragments.length; i++) {
				vecNames.add(dd.strFragments[i]);
			}
...
//		System.out.println(vecNames.size());
	...
	public static void main(String[] args) {
		DescriptorData dd=new DescriptorData();
		for (int i=0;i<dd.strFragments.length;i++) {
			System.out.println((i+1)+"\t"+dd.strFragments[i]);
		}
